...
class FaceDetector {
    ...
    async init() {
        try {
            ...
            this.modeIndicator.textContent = 'Ready - Click Start';
    ...
    analyzeFace(landmarks, box) {
        ...
        const foreheadClear = isFemale ? true : foreheadToEyebrow > expectedForeheadHeight * 0.5;
        const hasBangs = foreheadToEyebrow < expectedForeheadHeight * 0.35;

        // Allow bangs for female students - only flag for males
        const significantBangs = !isFemale && foreheadToEyebrow < expectedForeheadHeight * 0.2;
    ...
    detectEarrings(jaw, box) {
        try {
            ...
            for (const ear of earRegions) {
                ...
                // Flag if significant bright/metallic spots or if region color differs greatly from skin
                if (brightRatio > 0.15 || metallicRatio > 0.12 || (skinDist > 80 && (brightRatio > 0.08 || metallicRatio > 0.06))) {
